...
- **Breaking:** the `@solana/web3.js` peer dependency is now `^1.99.0`; 1.99.0 is the first stable release that understands V1 transactions.
...
- Converting a V1 message to `@solana/web3.js` objects (`toWeb3JsMessage`, `toWeb3JsMessageFromInput`) throws for a priority fee above 2^53 lamports, which `@solana/web3.js` models as a number and cannot represent; `umi.transactions.create` keeps such fees exact.
