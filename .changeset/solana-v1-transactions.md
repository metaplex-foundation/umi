...
- `TransactionBuilder` resolves its version like its fee payer: an explicit `setVersion()` wins, then the transaction factory's new optional `defaultVersion`, then V0, and `getVersion(context)` returns the result. Set the default for every builder with `web3JsTransactionFactory({ defaultTransactionVersion: 1 })` or `createUmi(endpoint, { defaultTransactionVersion: 1 })`; `umi.transactions.create()` still requires an explicit version.
