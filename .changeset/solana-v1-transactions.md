...
- `TransactionV1Config` values are validated when a V1 message is serialized (`assertValidTransactionV1Config`): the compute unit limit must be an integer between 0 and 1,400,000, the loaded accounts data size limit an integer between 0 and 67,108,864, and the heap size a multiple of 1,024 between 32,768 and 262,144, so invalid values fail with a clear error instead of being silently clamped or rejected by the node with an unrelated message.
