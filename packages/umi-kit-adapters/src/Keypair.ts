...
export function extractEd25519SecretKeyFromPkcs8(pkcs8: Uint8Array, publicKey: Uint8Array): Uint8Array {
 ...
 */
export async function fromKitKeypair(kitKeypair: CryptoKeyPair): Promise<Keypair> {
    // Export the public key as raw bytes
    const publicKeyBuffer = await crypto.subtle.exportKey('raw', kitKeypair.publicKey);
    // Export the private key as pkcs8
    const secretKeyPkcs8Buffer = await crypto.subtle.exportKey('pkcs8', kitKeypair.privateKey);
    ...
    const secretKeyByteArray = extractEd25519SecretKeyFromPkcs8(secretKeyPkcs8ByteArray, publicKeyByteArray);
    return {
