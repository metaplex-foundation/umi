...
import { createBaseUmi, generateSigner } from '@metaplex-foundation/umi';
...
test('it can roundtrip a public key back to Umi', async (t) => {
  ...
  t.deepEqual(
    publicKey.toString(),
    fromKitAddress(toKitAddress(publicKey)).toString()
  );
});
...
test('it can roundtrip a public key back to Kit', async (t) => {
  ...
  t.deepEqual(
    publicKey.toString(),
    toKitAddress(fromKitAddress(publicKey)).toString()
  );
});
