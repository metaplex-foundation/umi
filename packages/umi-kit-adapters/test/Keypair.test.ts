...
import { createUmi, generateSigner, publicKeyBytes } from '@metaplex-foundation/umi';
import { web3JsEddsa } from '@metaplex-foundation/umi-eddsa-web3js';
import { extractEd25519SecretKeyFromPkcs8, fromKitKeypair, toKitKeypair } from '../src';
...
test('fromKitKeypair converts Kit CryptoKeyPair to umi Keypair and back', async (t) => {
    ...
    t.deepEqual(publicKeyBytes(convertedKeypair.publicKey), publicKeyBytes(originalKeypair.publicKey));
...
test('toKitKeypair converts umi Keypair to Kit CryptoKeyPair and back', async (t) => {
    ...
    const exportedPublicKey = new Uint8Array(await crypto.subtle.exportKey('raw', kitKeypair.publicKey));
    const exportedSecretKeyPkcs8 = new Uint8Array(await crypto.subtle.exportKey('pkcs8', kitKeypair.privateKey));
    const exportedSecretKey = extractEd25519SecretKeyFromPkcs8(exportedSecretKeyPkcs8, exportedPublicKey);
