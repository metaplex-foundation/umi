...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <div>
    <h1>Umi Irys Uploader Test (Vite)</h1>
    <div class="card">
    ...
    <p class="read-the-docs">
      This tests that the umi-uploader-irys package can be loaded in a Vite-bundled app
    </p>
  </div>
`;
...
document.querySelector<HTMLButtonElement>('#test-btn')!.addEventListener('click', async () => {
  ...
  try {
    ...
    if (umi.uploader) {
      ...
      log('🎉 SUCCESS: All tests passed! The Irys uploader loaded correctly in Vite.');
    } else {
